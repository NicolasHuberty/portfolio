...
import { ArrowLeft, ExternalLink, Calendar, Users, Clock, Circle, ChevronLeft, ChevronRight } from "lucide-react"
...
const projectDetails: Record<string, {
    ...
    emate: {
        ...
        client: "Legal Sector",
        year: "2024",
        context: "Law firms struggle to find relevant information across **all legal sources** (doctrine, jurisprudence, legislation). Traditional tools are limited to keyword search and often miss the nuanced connections needed for complex cases.",
        challenge: "Create an AI system capable of 'reasoning' like a lawyer—searching broadly, filtering intelligently, and synthesizing information from multiple document types into a cohesive, cited answer.",
        solution: "Developed a sophisticated **Smart Agent architecture** that emerged after long iteration loops. The system uses **Python** with comprehensive AI libraries, persistent storage on **S3**, and a managed **Qdrant** vector database. It orchestrates multiple reasoning steps to analyze the full spectrum of legal documentation, not just case precedents.",
        results: [
    ...
    swet: {
        ...
        client: "SWET BXL",
        year: "2023-Present",
        context: "SWET BXL is a Brussels-based **artisan producer of natural hot sauces**. They manage complex flows ranging from **batch production** to **omnichannel distribution** (B2B shops & B2C e-commerce).",
        challenge: "Legacy tools couldn't handle the integrated needs of **manufacturing (MRP)**, inventory, and automated invoicing for their growing wholesale and retail operations.",
        solution: "Collaborated with **Tom Heyerick** to deploy a tailored **Odoo.sh** solution. We implemented and customized core modules (**Inventory, Accounting, Website**) and established automated payment & invoicing workflows, fully replacing the previous system.",
        results: [
    ...
    docuralis: {
        ...
        year: "2025",
        context: "Modern AI needs to go beyond static document search. Professionals require **autonomous agents** that can actively fetch external data (Web, APIs, Public Libraries) and synthesize it with internal knowledge.",
        challenge: "Orchestrating **Domain-Specific Agents** with distinct capabilities (Tools) while maintaining security. The goal was to implement the **Model Context Protocol (MCP)** to standardize how agents interact with the world.",
        solution: "Built a hybrid **RAG + Agentic Platform**. It enables the creation of specialized agents equipped with tools: **Web Search**, **API Connectors** (e.g., medical/legal databases), and **Private RAG**. Agents can plan, execute API calls, and reason across mixed data sources.",
        results: [
    ...
    ila: {
        ...
        thesis: {
            ...
            pdfUrl: "https://thesis.dial.uclouvain.be/server/api/core/bitstreams/a36a80b6-b3d5-4106-8a56-2616fd24bfdd/content",
            thesisUrl: "https://thesis.dial.uclouvain.be/entities/masterthesis/4151ba15-8019-4d4a-a4e0-a6c09bde60af",
        }
    },
    infrastructure: {
        duration: "Ongoing",
        ...
        year: "2022-Present",
        context: "Managing multiple projects requires reliable infrastructure with automated deployments, monitoring, and scalability.",
        challenge: "Build a private cloud infrastructure that can host all personal and client projects with enterprise-grade reliability and security.",
        solution: "Deployed a Kubernetes cluster with Terraform, implemented GitOps with ArgoCD, set up monitoring with Prometheus/Grafana, and automated SSL and DNS management.",
        results: [
...
function renderBlockContent(text: string) {
    ...
    return text.split('\n').map((line, index) => {
        ...
        if (trimmed.startsWith('- ')) {
            return (
                <div key={index} className="flex items-start gap-2 pl-4 mb-1">
                    <span className="text-zinc-400 mt-2 text-[6px]">●</span>
                    <span className="text-zinc-600 dark:text-slate-400 leading-relaxed">
        ...
        if (/^\d+\.\s/.test(trimmed)) {
            return (
                <div key={index} className="mb-1 mt-3 font-medium text-zinc-800 dark:text-slate-200">
        ...
        return (
            <p key={index} className="mb-2 text-zinc-600 dark:text-slate-400 leading-relaxed">
                {parseMarkdown(line)}
            </p>
        );
    });
...
export default function ProjectDetailClient({ project }: { project: Project }) {
    ...
    const allImages = details?.gallery ? [...project.screenshots, ...details.gallery] : project.screenshots
            ...
            <div className={`relative bg-gradient-to-br ${project.color} py-20`}>
                ...
                <div className="container mx-auto px-6 lg:px-8 relative">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                        <div>
                            ...
                            {project.logo && (
                                <motion.div
                                    ...
                                    className="w-20 h-20 rounded-2xl bg-white shadow-xl flex items-center justify-center p-3 mb-6"
                                >
                                    <Image
                                        src={project.logo}
                        ...
                        >
                            {/* Status */}
                            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${project.status === "online"
                                ? "bg-white/20 text-white"
                                : "bg-white/10 text-white/70"
                                }`}>
                                <Circle className={`w-2.5 h-2.5 ${project.status === "online" ? "fill-emerald-400 text-emerald-400" : "fill-white/50 text-white/50"
                                    }`} />
                            ...
                            {project.link && (
                                <a
                                    ...
                                    className="inline-flex items-center gap-2 px-4 py-2 bg-white text-zinc-900 rounded-full font-medium hover:bg-white/90 transition-colors"
                                >
                                    Visit Site
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        ...
                        {hasImages && (
                            ...
                            >
                                <div className="relative aspect-[16/10] rounded-2xl overflow-hidden bg-zinc-100 dark:bg-slate-800 shadow-lg">
                                    <Image
                                    ...
                                    {allImages.length > 1 && (
                                        <>
                                            <button
                                                onClick={() => setCurrentImage((i: number) => i === 0 ? allImages.length - 1 : i - 1)}
                                            ...
                                            <button
                                                onClick={() => setCurrentImage((i: number) => i === allImages.length - 1 ? 0 : i + 1)}
                                    ...
                                    <div className="flex gap-3 mt-4 overflow-x-auto pb-2">
                                        {allImages.map((img: string, i: number) => (
                                            <button
                                                ...
                                                className={`relative w-20 h-14 rounded-lg overflow-hidden flex-shrink-0 ${i === currentImage ? "ring-2 ring-indigo-500" : "opacity-60 hover:opacity-100"
                                                    } transition-all`}
                                            >
                        ...
                        {details?.context && (
                            ...
                            >
                                ...
                                <div>
                                    {renderBlockContent(details.context)}
                                </div>
                            </motion.div>
                        )}
                        ...
                        {details?.challenge && (
                            ...
                            >
                                ...
                                <div>
                                    {renderBlockContent(details.challenge)}
                                </div>
                            </motion.div>
                        )}
                        ...
                        {details?.solution && (
                            ...
                            >
                                ...
                                <div>
                                    {renderBlockContent(details.solution)}
                                </div>
                            </motion.div>
                        )}
                                ...
                                <ul className="space-y-3">
                                    {details.results.map((result: string, i: number) => (
                                        <li key={i} className="flex items-start gap-3">
                                            ...
                                            <span className="text-zinc-600 dark:text-slate-400">{parseMarkdown(result)}</span>
                                        </li>
                                    ))}
                                </ul>
                            </motion.div>
                        ...
                        {details?.thesis && (
                            <motion.div
                                ...
                                className="rounded-2xl border-2 border-violet-200 dark:border-violet-500/30 bg-gradient-to-br from-violet-50 to-indigo-50 dark:from-violet-500/10 dark:to-indigo-500/10 p-8"
                            >
                                <div className="flex items-center gap-3 mb-4">
                                    <span className="px-3 py-1 rounded-full bg-violet-100 dark:bg-violet-500/20 text-violet-700 dark:text-violet-300 text-sm font-medium">
                                        📄 Master&apos;s Thesis
                                    </span>
                                ...
                                <div className="mb-6">
                                    <h3 className="text-sm font-semibold text-zinc-700 dark:text-slate-300 uppercase tracking-wide mb-2">
                                        Abstract
                                    </h3>
                                    <p className="text-zinc-600 dark:text-slate-400 leading-relaxed text-sm">
                                        {details.thesis.abstract}
                                    </p>
                                </div>

                                <div className="mb-6">
                                    <h3 className="text-sm font-semibold text-zinc-700 dark:text-slate-300 uppercase tracking-wide mb-3">
                                        Key Contributions
                                    </h3>
                                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {details.thesis.contributions.map((contribution: string, i: number) => (
                                            <li key={i} className="flex items-start gap-2 p-3 rounded-xl bg-white dark:bg-slate-800 border border-zinc-100 dark:border-slate-700">
                                                <span className="text-violet-500 mt-0.5">✓</span>
                                                <span className="text-sm text-zinc-700 dark:text-slate-300">{contribution}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-700 text-zinc-900 dark:text-white border border-zinc-200 dark:border-slate-600 rounded-lg font-medium hover:bg-zinc-50 dark:hover:bg-slate-600 transition-colors"
                                    >
                                        🔗 View on Repository
                                    </a>
                                </div>
                    ...
                    <div className="lg:col-span-1">
                        ...
                        >
                                ...
                                <dl className="space-y-4">
                                    {details?.year && (
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <dt className="text-xs text-zinc-500 dark:text-slate-500">Year</dt>
                                                <dd className="text-zinc-900 dark:text-white font-medium">{details.year}</dd>
                                            </div>
                                        </div>
                                    )}
                                    {details?.duration && (
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <dt className="text-xs text-zinc-500 dark:text-slate-500">Duration</dt>
                                    ...
                                    {details?.team && (
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <dt className="text-xs text-zinc-500 dark:text-slate-500">Team</dt>
                                                <dd className="text-zinc-900 dark:text-white font-medium">{parseMarkdown(details.team)}</dd>
                                            </div>
                                        </div>
                                    )}
                                    {details?.role && (
                                        <div className="pt-3 border-t border-zinc-200 dark:border-slate-700">
                                            <dt className="text-xs text-zinc-500 dark:text-slate-500 mb-1">My Role</dt>
                                            <dd className="text-zinc-900 dark:text-white font-medium">{parseMarkdown(details.role)}</dd>
                                        </div>
                                    )}
                                    {details?.client && (
                                        <div className="pt-3 border-t border-zinc-200 dark:border-slate-700">
                                            <dt className="text-xs text-zinc-500 dark:text-slate-500 mb-1">Client / Sector</dt>
                                            <dd className="text-zinc-900 dark:text-white font-medium">{details.client}</dd>
                                        </div>
                                    )}
                                </dl>
                            </div>
                            ...
                            {/* Impact */}
                            <div className={`rounded-2xl bg-gradient-to-br ${project.color} p-6`}>
                            ...
                            {project.link && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center justify-center gap-2 w-full px-6 py-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl font-medium hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors"
                                >
                                    Visit Live Project
                                    <ExternalLink className="w-4 h-4" />
                                </a>
